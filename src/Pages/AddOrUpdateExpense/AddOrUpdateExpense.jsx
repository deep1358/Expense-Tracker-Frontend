...
const AddOrUpdateExpense = () => {
  ...
  const { creatingExpense, focusedExpense, updatingExpense, gettingExpense } = useSelector(
    (state) => state.expense
  );
      ...
      if (id !== undefined) {
        ...
        if (Object.keys(focusedExpense)?.length > 0) {
          form.setValues({
            ...
            date: new Date(focusedExpense.year, focusedExpense.month - 1, focusedExpense.day),
  ...
  useEffect(() => {
    if (creatingExpense || updatingExpense) dispatch(toggleLoadingOverlay(true));
  ...
  return (
    <>
      <Container size={420}>
        <Title align="center">{id === undefined ? 'Add Expense' : 'Update Expense'}</Title>
        <Paper withBorder shadow="md" p={30} mt={30} size="md" radius="md" mx="auto">
          {id !== undefined && gettingExpense ? (
            ...
            <form onSubmit={form.onSubmit(handleSaveOrUpdate)}>
              ...
              <Group position="apart" mt="lg">
                <Button
                  style={id === undefined && matches ? { width: '47%' } : { width: '100%' }}
                  type="submit"
                >
